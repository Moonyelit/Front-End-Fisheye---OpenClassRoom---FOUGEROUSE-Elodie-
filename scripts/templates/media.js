...
function mediaTemplate(data) {
    ...
    function getMediaCardDOM() {
        ...
        // Bouton like (#11 - Icône avec aria-label) - un like maximum par média (étape 8)
        ...
        likesIcon.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            // Un seul like par média : ignorer si déjà liké
            if (likesIcon.getAttribute('aria-pressed') === 'true') return;

            // Incrémenter les likes
            const currentLikes = parseInt(likesCount.textContent);
            likesCount.textContent = currentLikes + 1;
            article.setAttribute("data-likes", currentLikes + 1);
            likesIcon.setAttribute("aria-pressed", "true");
